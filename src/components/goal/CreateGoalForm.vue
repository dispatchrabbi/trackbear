...
import { GOAL_CADENCE_UNIT_INFO } from 'src/lib/goal';
...
const emit = defineEmits(['goal:create', 'formSuccess']);
...
const validations = z
  .object({
    ...
    count: z
      ...
      .refine(v => formModel.type === GOAL_TYPE.TARGET ? v !== null : true, { message: 'A threshold is required for targets.'}),
    ...
    works: z.array(z.number({ invalid_type_error: 'Please select only valid projects.' }).positive({ message: 'Please select only valid projects.' })),
    tags: z.array(z.number({ invalid_type_error: 'Please select only valid tags.' }).positive({ message: 'Please select only valid tags.' })),
  });
